...
BEGIN;
ALTER TABLE `__PREFIX__unishop_order_extend`
    ADD COLUMN `express_company` varchar(30) NOT NULL DEFAULT '' COMMENT '快递公司' AFTER `express_number`;
COMMIT;
...
-- 新增商品销量视图
--
CREATE VIEW `__PREFIX__unishop_goods` AS SELECT o.createtime AS date_time,op.order_id,op.product_id,op.number,p.title AS product_name
FROM `__PREFIX__unishop_order` o
INNER JOIN `__PREFIX__unishop_order_product` op ON o.id = op.order_id
LEFT JOIN `__PREFIX__unishop_product` p ON op.product_id = p.id
...
-- 新增收入统计视图
--
CREATE VIEW `__PREFIX__unishop_income` AS
SELECT o.createtime AS date_time, o.total_price AS order_amount
FROM `__PREFIX__unishop_order` o where o.status = 1 and o.have_paid != 0;

--
-- 2.3.0
-- 微信小程序jscode2session的session_key
--
alter table __PREFIX__unishop_user_extend add session_key varchar(100) null comment '微信小程序jscode2session的session_key';
